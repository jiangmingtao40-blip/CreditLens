...
-- 创建数据库
CREATE DATABASE IF NOT EXISTS `credit_report` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE `credit_report`;
...
SELECT 
  ...
  u.free_count,
  u.total_commission,
  u.available_commission,
  COUNT(DISTINCT r.id) AS total_reports,
  COUNT(DISTINCT CASE WHEN r.status = 'completed' THEN r.id END) AS completed_reports,
  SUM(CASE WHEN r.is_free = 0 THEN r.cost_amount ELSE 0 END) AS total_cost,
  COUNT(DISTINCT i.id) AS total_invites
FROM credit_user u
LEFT JOIN credit_report r ON u.id = r.user_id AND r.is_deleted = 0
LEFT JOIN credit_invite i ON u.id = i.user_id
...
BEGIN
  ...
  DECLARE v_invite_code VARCHAR(32);
  
  -- 查询用户是否存在
  SELECT id INTO p_user_id FROM credit_user WHERE openid = p_openid LIMIT 1;
  ...
  IF p_user_id IS NULL THEN
    ...
    -- 生成邀请码
    SET v_invite_code = CONCAT('INV', UPPER(SUBSTRING(MD5(CONCAT(p_openid, NOW())), 1, 8)));
    
    -- 查询邀请人
    IF p_invite_code IS NOT NULL AND p_invite_code != '' THEN
      SELECT id INTO v_invited_by FROM credit_user WHERE invite_code = p_invite_code LIMIT 1;
    END IF;
    
    -- 插入用户
    INSERT INTO credit_user (openid, nickname, avatar, invite_code, invited_by, free_count)
    VALUES (p_openid, p_nickname, p_avatar, v_invite_code, v_invited_by, 3);
    
    SET p_user_id = LAST_INSERT_ID();
    
    -- 如果有邀请人，创建邀请记录
    IF v_invited_by IS NOT NULL THEN
      INSERT INTO credit_invite (user_id, invite_user_id, status)
      VALUES (v_invited_by, p_user_id, 'pending');
    END IF;
  ELSE
    -- 老用户登录
    SET p_is_new = 0;
    UPDATE credit_user 
    SET nickname = p_nickname, avatar = p_avatar, last_login_time = NOW()
    WHERE id = p_user_id;
  END IF;
END //
DELIMITER ;
...
-- 扣减免费次数存储过程
DELIMITER //
CREATE PROCEDURE `sp_deduct_free_count`(
  IN p_user_id BIGINT,
...
BEGIN
  ...
  SELECT free_count INTO v_free_count FROM credit_user WHERE id = p_user_id;
  
  IF v_free_count > 0 THEN
    -- 有免费次数，扣减
    UPDATE credit_user SET free_count = free_count - 1 WHERE id = p_user_id;
    SET p_success = 1;
    SET p_is_free = 1;
  ELSE
    -- 无免费次数
